...
namespace Vtex.RabbitMQ.Messaging
{
    public class RabbitMQConsumer<T> : IQueueConsumer where T : class
    {
        ...
        public async Task Start()
        {
            ...
            await Task.Factory.StartNew(async () => await ManageConsumersLoop(token), token);
        }
        ...
        protected async virtual Task ManageConsumersLoop(CancellationToken cancellationToken)
        {
                    ...
                    for (var i = 1; i <= scalingAmount; i++)
                    {
                        ...
                        await Task.Factory.StartNew(async () =>
                        {
                            try
                            {
                                ...
                                {
                                    await consumerWorker.DoConsume();
                                }
                            }
